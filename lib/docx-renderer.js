  ...
  return traverse(values, (value) => {
    ...
    try {
      ...
      if (!content.document || !content.document.nodes) {
        console.log('found json but not text editor');
        return Promise.resolve(value);
      }

      console.log('text editor found');
      console.log('nodes before: ', content.document.nodes);
      ...
      return Promise.all(nodePromises)
        .then(nodes => {
          ...
          console.log('nodes after: ', content.document.nodes);
          return JSON.stringify(content);
        });
    } catch (e) {
      // not json, do nothing
      console.log('found standard string');
      return Promise.resolve(value);
    }
  });
};
